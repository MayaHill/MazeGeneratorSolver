...
void check_path(solver_t *d, coords_t *new, int *found)
{
    if (d->temp.x < 0 || d->temp.x >= d->height)
        return;
    if (d->temp.y < 0 || d->temp.y >= d->width)
        return;
    if (d->map[d->temp.x][d->temp.y] == '*') {
        if ((*found) == 0) {
            d->map[d->temp.x][d->temp.y] = 'o';
            new->x = d->temp.x;
            new->y = d->temp.y;
        }
        (*found) += 1;
...
int end_conditions(solver_t *dante, int found, coords_t *coords, coords_t *new)
{
    if (!found) {
        dante->map[coords->x][coords->y] = '2';
        return (0);
    }
    if (new->x == dante->height - 1 && new->y == dante->width - 1) {
        dante->map[new->x][new->y] = 'o';
        free(coords);
        return (1);
    }
    if (found > 0) {
